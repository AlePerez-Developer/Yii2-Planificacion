...
use app\modules\Planificacion\services\OrganismoService;
use Yii;
...
class OrganismoController extends BaseController
{
    public function __construct(
        $id,
        $module,
        private OrganismoService $service,
        $config = []
    ) {
    ...
    private function cargarFormulario(): OrganismoForm
    {
        $form = new OrganismoForm();
    ...
    {
        $id = (string)Yii::$app->request->post('idOrganismo', '');
        if ($id === '') {
            throw new ValidationException(
                Yii::$app->params['ERROR_ENVIO_DATOS'],
                'No se recibió el organismo.',
                400
            );
        }
        return $id;
