...
use app\modules\Planificacion\services\PartidaService;
use Yii;
...
class PartidaController extends BaseController
{
    ...
    public function actionVerificarCombinacion(): bool
    {
        return $this->service->verificarCombinacion(
            (string)Yii::$app->request->post('idPartida', '00000000-0000-0000-0000-000000000000'),
            (string)Yii::$app->request->post('idFuente', ''),
            (string)Yii::$app->request->post('idOrganismo', '')
        );
    }

    private function cargarFormulario(): PartidaForm
    {
    ...
    {
        $id = (string)Yii::$app->request->post('idPartida', '');
        if ($id === '') {
            throw new ValidationException(
                Yii::$app->params['ERROR_ENVIO_DATOS'],
                'No se recibió la partida.',
                400
            );
        }
        return $id;
