...
class ItemsCatalogadosService
{
    public function listarTodo(
        ...
        $items = Item::find()->alias('IT')
            ->select([
                ...
                'IdSigma' => 'IC.IdSigma',
                'SigmaClase' => 'CS.Clase',
                'SigmaDescripcion' => 'CS.Descripcion',
                'SigmaRamaComercial' => 'CS.RamaComercial',
                'SigmaEspecificacion' => 'CS.Especificacion',
            ...
            ->innerJoin(['IC' => ItemCatalogado::tableName()], 'IC.IdItem = IT.IdItem')
    ...
    public function listarSigma(int $formulario): array
    {
        $data = CatalogoSigma::find()
            ->select([
                'IdSigma',
                'Clase',
                'Descripcion',
            ...
            ->andWhere(['like', 'IdGasto', $formulario . '%', false])
    ...
    public function guardar(
        ...
        try {
            ...
            $catalogado = new ItemCatalogado([
                'IdItem' => $item->IdItem,
                'IdSigma' => $form->idSigma,
            ]);
            $this->guardarModelo($catalogado);
        ...
        return ResponseHelper::success($item, 'Ítem catalogado guardado correctamente.');
    }
    ...
    public function actualizar(
        ...
        try {
            ...
            $catalogado = ItemCatalogado::findOne(['IdItem' => $item->IdItem]);
            if ($catalogado === null) {
                $catalogado = new ItemCatalogado(['IdItem' => $item->IdItem]);
            }
            $catalogado->IdSigma = $form->idSigma;
            $this->guardarModelo($catalogado);
            ...
            foreach ($existentes as $idOperacion => $igo) {
                if (!isset($asignaciones[$idOperacion])) {
                    if ($igo->CodigoEstado !== Estado::ESTADO_ELIMINADO) {
                        $igo->eliminar();
        ...
        return ResponseHelper::success($item, 'Ítem catalogado actualizado correctamente.');
    }
    ...
    public function obtenerModelo(
        ...
        $catalogado = ItemCatalogado::findOne(['IdItem' => $item->IdItem]);
        $sigma = $catalogado ? CatalogoSigma::findOne($catalogado->IdSigma) : null;
        $gestion = ItemGestion::find()
            ->where(['IdItem' => $item->IdItem, 'IdGestion' => $idGestion])
    ...
    protected function operacionesPorItems(
        array $ids,
        string $idUnidad,
        string $idGestion,
        string $idEstadoPoa
    ...
    protected function normalizarAsignaciones(array $asignaciones): array
    {
        $normalizadas = [];
     ...
     * @return Operacion[]
     */
    protected function obtenerOperacionesValidas(
        array $ids,
        string $idUnidad,
        string $idGestion,
        string $idEstadoPoa
     ...
     */
    protected function validarTechos(
        array $asignaciones,
        array $operaciones,
        float $precio,
        string $idGestion,
    ...
    private function validarCatalogos(ItemCatalogadoForm $form): void
    {
        ...
        if (!str_starts_with((string)$sigma->IdGasto, (string)$form->formulario)) {
            throw new ValidationException(
                Yii::$app->params['ERROR_ENVIO_DATOS'],
                'El ítem SIGMA no corresponde al formulario seleccionado.',
                422
            );
        }
    ...
    protected function obtenerItemValidado(string $idItem, int $formulario): Item
    {
    ...
    protected function guardarModelo($modelo): void
    {
        if (!$modelo->validate()) {
            throw new ValidationException(
