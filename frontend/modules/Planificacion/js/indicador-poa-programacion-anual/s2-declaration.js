...
$(document).ready(function () {
    programacionPoaAnual_s2ObjEspecifico.select2({
        theme: 'bootstrap4',
    ...
    $.ajax({
        url: 'index.php?r=Planificacion/indicador-poa-programacion-anual/listar-objetivos-especificos-s2',
        method: 'POST',
        dataType: 'json',
        success: function (response) {
            programacionPoaAnual_s2ObjEspecifico.empty().append(new Option('', '', false, false));
            (response.data || []).forEach(item => {
                const option = new Option(item.text, item.id, false, false);
                $(option).data({compuesto: item.compuesto, producto: item.producto});
                programacionPoaAnual_s2ObjEspecifico.append(option);
            });
