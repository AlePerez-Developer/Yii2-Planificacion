$(document).ready(function () {
    ...
    const baseUrl = 'index.php?r=Planificacion/foda-institucion/';
    let idFoda = EMPTY;
    ...
    $('#btnGuardar').on('click', async function () {
        if (!$('#formFodaInstitucion').valid()) return;
        ...
        try {
            await ajaxPromise({
                ...
                reloadTable: dt_fodaInstitucion
            });
        } catch (error) {
            console.error(error);
        }
    ...
    $('#tablaFodaInstitucion').on('click', '.btn-edit', function () {
        const row = dt_fodaInstitucion.row($(this).closest('tr')).data();
        $.ajax({
            url: baseUrl + 'buscar',
            method: 'POST',
            dataType: 'json',
    ...
    $('#tablaFodaInstitucion').on('click', '.btn-toggle-estado', function () {
        const row = dt_fodaInstitucion.row($(this).closest('tr')).data();
        ejecutar('cambiar-estado', row.IdFoda, 'Estado actualizado.');
    });

    $('#tablaFodaInstitucion').on('click', '.btn-delete', function () {
        const row = dt_fodaInstitucion.row($(this).closest('tr')).data();
        Swal.fire({
            icon: 'warning',
    ...
    function limpiar() {
        idFoda = EMPTY;
        $('#formFodaInstitucion')[0].reset();
        $('#formFodaInstitucion').validate().resetForm();
    }
});
