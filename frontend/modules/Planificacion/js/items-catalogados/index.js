$(document).ready(function () {
    ...
    const tabla = $('#tablaListaItemsCatalogados');
    let idItem = EMPTY;
    let operaciones = [];
    ...
    $('#btnReportePdf').on('click', function () {
        window.open($('#itemsCatalogadosPage').data('reporte'), '_blank');
    });

    $('#btnSeleccionarSigma').on('click', function () {
        $('#modalSigma').modal('show');
        if (dt_sigma) {
            dt_sigma.columns.adjust();
        }
    });

    $('#tablaSigma').on('click', '.btn-select-sigma', function () {
        const row = dt_sigma.row($(this).closest('tr')).data();
        seleccionarSigma(row);
        $('#modalSigma').modal('hide');
    });

    $('#btnGuardar').on('click', async function () {
        if (!$('#formItemCatalogado').valid()) return;
        if (!$('#idSigma').val()) {
            MostrarMensaje('error', 'Debe seleccionar un ítem SIGMA.');
            return;
        }
        ...
        datos.append('idSigma', $('#idSigma').val());
        datos.append('idFuente', items_s2Fuente.val() || '');
        datos.append('idOrganismo', items_s2Organismo.val() || '');
        datos.append('idFuenteUniversitaria', items_s2FuenteUniversitaria.val() || '');
        datos.append('idUnidadMedida', items_s2UnidadMedida.val() || '');
        ...
        await ajaxPromise({
            ...
            reloadTable: dt_itemsCatalogados
        });
    });
    ...
    tabla.on('click', 'td.details-control, td:not(:last-child)', function (e) {
        ...
        const row = dt_itemsCatalogados.row(tr);
        if (!row.data()) return;
    ...
    tabla.on('click', '.btn-delete', function (e) {
        e.stopPropagation();
        const row = dt_itemsCatalogados.row($(this).closest('tr')).data();
        Swal.fire({
            icon: 'warning',
            title: 'Eliminar ítem',
    ...
    function seleccionarSigma(row) {
        $('#idSigma').val(row.IdSigma);
        $('#descripcion').val(row.Descripcion || '');
        $('#detalleSigma').html(`
            <dl class="row mb-0">
                <dt class="col-sm-3">Clase</dt><dd class="col-sm-9">${row.Clase || ''}</dd>
                <dt class="col-sm-3">Descripción</dt><dd class="col-sm-9">${row.Descripcion || ''}</dd>
                <dt class="col-sm-3">Rama comercial</dt><dd class="col-sm-9">${row.RamaComercial || ''}</dd>
                <dt class="col-sm-3">Especificación</dt><dd class="col-sm-9">${row.Especificacion || ''}</dd>
            </dl>
        `).show();
    }
    ...
    async function llenarFormulario(data) {
        idItem = data.IdItem;
        if (data.Sigma) {
            seleccionarSigma(data.Sigma);
        }
        $('#descripcion').val(data.Descripcion || '');
        $('#precio').val(data.Precio || '');
        items_s2Fuente.val(data.IdFuente).trigger('change');
        await esperarOpciones(items_s2Organismo);
        items_s2Organismo.val(data.IdOrganismo).trigger('change');
        await esperarOpciones(items_s2FuenteUniversitaria);
        items_s2FuenteUniversitaria.val(data.IdFuenteUniversitaria).trigger('change.select2');
        items_s2UnidadMedida.val(data.IdUnidadMedida).trigger('change.select2');
        const cantidades = {};
    ...
    function htmlOperaciones(lista) {
        ...
        const filas = lista.map(op => `
            <tr>
                ...
                <td class="text-right">${formatoMonto(op.Cantidad)}</td>
            </tr>
        `).join('');
        return `
    ...
    function limpiar() {
        ...
        $('#formItemCatalogado').validate().resetForm();
        items_s2Fuente.val(null).trigger('change');
        items_s2UnidadMedida.val(null).trigger('change.select2');
        renderAccordion();
    }
});
